...
from utils.utils import getRt, transform, get_obb, get_box3d_min_max, get_iou_obb, class_filter, uv2xyz, \
    generate_scannet_seg
import numpy as np
import argparse
import clip
...
def data_info_extractor(scannet_frames_25k, scan_frame_id):
    ...
    # scaling factor
    SCALE_FACTOR = 1000
    scene_id = scan_frame_id[5:9]
    frame_id = scan_frame_id[10:12]
    an_id = scan_frame_id[4:6]
    color_img = os.path.join(scannet_frames_25k, "scene{}_{}".format(scene_id, an_id), "color",
                             "{}.jpg".format(frame_id))
...
def cal_iou(region_lst, center_lst, d3boxes, eval_question, scene):
    ...
    x_min_1, x_max_1, y_min_1, y_max_1, z_min_1, z_max_1 = get_box3d_min_max(
        d3boxes[int(torch.argmax(logits, dim=1).cpu().detach().numpy())])
    # 显示预测的物体
    region_lst[int(torch.argmax(logits, dim=1).cpu().detach().numpy())].show()
    # 真实的物体的box的坐标
    arr = np.load("data/scannet_v4/scans/{}/npy/{}_bbox.npy".format(scene, scene))
...
iou=cal_iou(region_lst, center_lst, d3boxes, ["The trash can", 1],"{}".format("scene0707_00_000000"))
#
