...
def main():
    ...
    if cfg.data.scannetv2_labels_table.type in ('MultiImageMixDataset', 'ClassBalancedDataset',
                               'RepeatDataset', 'ConcatDataset'):
        cfg.data.test.pipeline = get_loading_pipeline(
            cfg.data.scannetv2_labels_table.dataset.pipeline)
    else:
        cfg.data.test.pipeline = get_loading_pipeline(cfg.data.scannetv2_labels_table.pipeline)
