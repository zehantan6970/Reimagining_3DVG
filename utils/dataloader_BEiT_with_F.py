...
import numpy as np
from transformers import BlipImageProcessor, BlipVisionModel,AutoImageProcessor,BeitModel
import torch

device = "cuda" if torch.cuda.is_available() else "cpu"
# model = BlipVisionModel.from_pretrained("Salesforce/blip-image-captioning-base").to(device)
# # model.eval()
...
model.load_state_dict(torch.load("/hy-tmp/pytorch_vit_68_0.009015.pt"))
def dataload(rootpath, scene_name=None):
    ...
    for i, scene_dir in enumerate(region_floders_listdirs):
        if scene_name == '{}{}{}'.format(scene_dir[5:9], scene_dir[10:12], scene_dir[13:]):
            ...
            # image_patch_features = model.(**image_processed).cpu().detach().numpy()
