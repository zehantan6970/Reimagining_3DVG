...
def dataload(rootpath, scene_name=None):
    ...
    for i, scene_dir in enumerate(region_floders_listdirs):
        if scene_name == '{}{}{}'.format(scene_dir[5:9], scene_dir[10:12], scene_dir[13:]):
            ...
            image_processed = processor(image_patches,return_tensors="pt").to(device)
            image_patch_features = model.get_image_features(**image_processed).cpu().detach().numpy()
        ...
        if scene_name == None:
            ...
            for in_dir in in_listdirs:
                ...
                image_patches.append(image)
            # 批量处理1个region文件夹中的图片
            inputs = processor(image_patches, return_tensors="pt").to(device)
            image_patch_features = model.get_image_features(**inputs).cpu().detach().numpy()
            all_scene_patches['{}{}{}'.format(scene_dir[5:9], scene_dir[10:12], scene_dir[13:])] = image_patch_features
            print("feature extractor complete {}/{}".format(i + 1, region_floders_listdirs_len))
    # --------------------- #
    # 导入region 3d
...
def dataload_for_eval(region_lst):
    inputs = processor(region_lst, return_tensors="pt").to(device)
    image_patch_features = model.get_image_features(**inputs).cpu().detach().numpy()
